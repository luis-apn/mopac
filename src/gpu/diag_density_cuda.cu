   ...
   return;
    ...
    extern "C" void MagmaDsyevd_Driver1(int ngpus, char opt1, char opt2, int n, REAL *eigenvecs, int m,
        	...
        	if (ngpus == 1){
        		...
        		magma_dsyevd_gpu(magma_vec_const(opt1), magma_uplo_const(opt2), n, eigenvecs_dev, m, eigvals, wa, n, work_tmp, lwork, iwork_tmp, liwork, info);
        		free(wa);
        		cudaFree(eigenvecs_dev);
        	}
        	else {
        		magma_dsyevd_m(ngpus, magma_vec_const(opt1), magma_uplo_const(opt2), n, eigenvecs, m, eigvals, work_tmp, lwork, iwork_tmp, liwork, info);
        	}
        	return;
        ...
        extern "C" void MagmaDsyevd_Driver2(int ngpus,char opt1, char opt2, int n, REAL *eigenvecs, int m,
        	...
        	if (ngpus == 1){
        		...
        		handleError(cudaMalloc((void **) &eigenvecs_dev, sizeof(double)*n*n));
        		handleError(cudaMemcpy(eigenvecs_dev, eigenvecs, sizeof(double)*n*n,cudaMemcpyHostToDevice));
        		magma_dsyevd_gpu(magma_vec_const(opt1), magma_uplo_const(opt2), n, eigenvecs_dev, m, eigvals, wa, n, work, lwork, iwork, liwork, info);
        		handleError(cudaMemcpy(eigenvecs,eigenvecs_dev,sizeof(double)*n*n,cudaMemcpyDeviceToHost));
        		free(wa);
        		cudaFree(eigenvecs_dev);
        	}
        	else {
        		magma_dsyevd_m(ngpus, magma_vec_const(opt1), magma_uplo_const(opt2), n, eigenvecs, m, eigvals, work, lwork, iwork, liwork, info);
        	}
        	return;
